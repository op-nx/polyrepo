...
- **Forward __unparsed__ args** to child Nx process
...
export async function populateGraphReport(
  ...
  const newHash = await computeOuterHash(config, workspaceRoot, pluginOptionsHash);
...
async function computeOuterHash(
  ...
  for (const entry of entries) {
    ...
    const dirtyFiles = await execGitOutput(['diff', '--name-only', 'HEAD'], repoPath);
...
function transformGraphForRepo(
  ...
  for (const [originalName, node] of Object.entries(rawGraph.graph.nodes)) {
    ...
    for (const [targetName, targetConfig] of Object.entries(node.data.targets ?? {})) {
      proxyTargets[targetName] = {
        ...
        outputs: rewriteOutputPaths(targetConfig.outputs, repoRelativeRoot, hostRoot),
  ...
  for (const [sourceName, deps] of Object.entries(rawGraph.graph.dependencies)) {
...
export const createDependencies: CreateDependencies<PolyrepoConfig> = async (
  ...
  for (const [repoAlias, repoReport] of Object.entries(report.repos)) {
    for (const dep of repoReport.dependencies) {
      ...
      if (context.projects[namespacedSource] && context.projects[namespacedTarget]) {
        dependencies.push({
